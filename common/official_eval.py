...
def evaluate(answers, predictions, skip_no_answer=False, debug=False):
    ...
    for qid, ground_truths in answers.items():
        ...
        is_exact = metric_max_over_ground_truths(
            exact_match_score, prediction, ground_truths)
        if not is_exact and debug:
            print('qid: %s\tprediction: %s\tgold: %s' % (qid, prediction, ' | '.join(ground_truths)))
